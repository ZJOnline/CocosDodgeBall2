...
export class TimerMgr
{
    ...
    add(interval:number, delay:number, repeat:number, cb:handler, is_updater:boolean = false):number
    {
        ...
        // console.log(`[TimerMgr]addPending, key=${key}`);
        return key;
    }
    ...
    private removeIter(key:number)
    {
        ...
        if(node)
        {
            ...
            // console.log(`[TimerMgr]removeIter, key=${key}`);
            return true;
        }
        return false;
    }
    ...
    private removePending(key:number)
    {
        ...
        {
            this.pool.push(node.data);
            // console.log(`[TimerMgr]removePending, key=${key}`);
            return true;
        }
        return false;
    }
    ...
    update(dt:number)
    {
        ...
        while(node)
        {
            ...
            if(node.data.elapsed >= node.data.delay + node.data.interval)
            {
                ...
                // console.log(`[TimerMgr]execHandler, key=${node.key}`);
                const next = node.next;
                node.data.times++;
