import { UIManager, UI_CONFIG } from "./UIManager"
...
export class UIBase extends cc.Component {
    ...
    protected addMask: boolean = false;
    ...
    */
    // onLoad()
    // {
    //     console.log("on_load 00000000000000000");
    // }
    ...
    __show__(...params: any[]): void {
        ...
        if (!overlay && this.addMask) {
            UIPoolMgr.get_inst().get_ui(UI_CONFIG.overlay_bg, gen_handler((bg_node: cc.Node): void => {
                if (!this.is_show || this.node.getChildByName(pop_overlay_bg)) {
                    UIPoolMgr.get_inst().put_ui(UI_CONFIG.overlay_bg, bg_node);
                    return;
                }
    ...
    /**关闭自身*/
    hide(): void {
        UIManager.get_inst().hide(this._ui_name);
    }
