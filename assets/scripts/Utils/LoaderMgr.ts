...
export class LoaderMgr {
    private static inst: LoaderMgr;
    private constructor() { };

    static get_inst(): LoaderMgr {
        if (!LoaderMgr.inst) {
            LoaderMgr.inst = new LoaderMgr();
        }
        return LoaderMgr.inst;
    }
    ...
    loadExternalAssets(urls: string[], cb: handler, types?: string[]) {
        ...
        cc.loader.load(loadTasks, (errs, res) => {
            console.log("loadExternalAssets from remote url");
            if (errs) {
    ...
    loadPrefabObj(url: string, cb: handler) {
        ...
        console.log("instantiate prefab:", url);
